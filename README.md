...
| `js/app.js` | Rendu des pages, navigation, animations, visionneuse |
...
| `#/agenda` | Agenda, Belgique seulement : les dates à venir (s'il y en a), puis les passées |
| `#/compagnie` | Histoire et équipe, chaque nom est un lien |
| `#/personne/<clé>` | Fiche personne : projets, puis bio si elle existe |
| `#/galerie` | Galerie, visionneuse au clic |
...
- Un spectacle a `troupe: "bruxelles"` ou `"montreal"`. `slides` = photos en plus pour le carrousel, sans répéter la photo du haut ni l'affiche ; sans `slides`, pas de carrousel.
...
- Une date = une ligne dans `SHOWS[].dates` (`day`, `time`, `venue`, `city`, `price`). L'agenda regroupe tout seul les soirs consécutifs en séries (« 17 et 18 avr. 2026 », « 24 au 26 mai 2024 »).
...
| Sujet | Choix |
|---|---|
| Navigation | Par hash (`#/spectacle/par-endroits`). La nouvelle page apparaît en fondu **par-dessus** l'ancienne, photo sur photo, jamais par le noir. |
...
| Accessibilité | Lien « Aller au contenu », un h1 par page, cibles tactiles de 44 px, focus gardé dans le menu et la visionneuse, `prefers-reduced-motion` respecté |
